...
class Solution {
public:
    int solve(int groupIndex, int groupElementStartIndexInArray, int m, int n, vector<int> &nums) {
      if (groupIndex == m-1) {
      ...
      for (int i = groupElementStartIndexInArray; i < n; ++i) {
        ...
        int largestForThisI = larger(currentGroupSum, minOfLargestGroupSumOfTheRest);
        currentGroupCacheHolder = smaller(currentGroupCacheHolder, largestForThisI);
      }
