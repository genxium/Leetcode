...
int dp[MAXN+1][MAXN+1][MAXN+1]; // "dp[l][r][k] == x" means that by having "k" preceding boxes of the same color as "boxes[l]", we can burst "boxes[l, ..., r]" and obtain at most "x" points
...
public:
  int solve(int l, int r, int k, vector<int>& boxes) {
    if (INVALID != dp[l][r][k]) return dp[l][r][k];
