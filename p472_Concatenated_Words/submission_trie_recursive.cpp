...
public:
  ...
  void insert(int idx, string &word) {
    if (idx == word.length()) {
      //printf("Ending insertion of word:%s\n", word.c_str());
      if (this->branches.find(TERMINATOR) == this->branches.end()) {
        this->branches[TERMINATOR] = new TrieNode();        
      }
      return;
    }
    char c = word[idx];
    if (this->branches.find(c) == this->branches.end()) {
      this->branches[c] = new TrieNode();
    }
    this->branches[c]->insert(++idx, word);
  }
  
  bool isOrigWordEnd() {
    return this->branches.find(TERMINATOR) != this->branches.end();
  }
  
  void isCompositeByExistingWords(int idx, string &word, int leafMatchCount, TrieNode* grandRoot, bool &ans, int level) {
    if (true == ans) {
      return;
    }
    
    if (idx == word.length()) {
      // printf("isCompositeByExistingWords this:%p, reaching word end of %s, with leafMatchCount:%d, level:%d\n", this, word.c_str(), leafMatchCount, level);
      if (
        this->isOrigWordEnd()
          && 
        leafMatchCount >= 1 // already matched with at least one word in the dictionary
      ) {
        ans = true;
      }
      return;
    ...
    char thatCh = word[idx];
    // printf("isCompositeByExistingWords this:%p, word[idx:%d]:%c of word:%s, with leafMatchCount:%d, level:%d\n", this, idx, thatCh, word.c_str(), leafMatchCount, level);  
    if (this->branches.find(thatCh) != this->branches.end()) {
      //printf("\tisCompositeByExistingWords continuing with deeper branch\n");  
      auto thatBranch = this->branches[thatCh]; 
      thatBranch->isCompositeByExistingWords(idx+1, word, leafMatchCount, grandRoot, ans, level+1);
    }
    
    if (this->isOrigWordEnd()) {
      //printf("\tisCompositeByExistingWords continuing with grandRoot\n"); 
      grandRoot->isCompositeByExistingWords(idx, word, leafMatchCount+1, grandRoot, ans, level+1);
    }
  }
};
...
public:
    vector<string> findAllConcatenatedWordsInADict(vector<string>& words) {
      ...
      // } puts("");
      vector<string> ans;
      TrieNode* grandRoot = new TrieNode();
      for (auto &word : words) {
        if (word.empty()) {
          // This is important to prevent "dead loop".
          continue;
        }
        bool currentIsCompositeByExistingWords = false;
        grandRoot->isCompositeByExistingWords(0, word, 0, grandRoot, currentIsCompositeByExistingWords, 0);
        if (currentIsCompositeByExistingWords) {
          ans.push_back(word);
        }
        int idx = 0;
        grandRoot->insert(idx, word);
      }
      return ans;
    }
};
