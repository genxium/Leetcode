#define MAXY 150
#define MAXX 150
bool cacheTouchesPacific[MAXY][MAXX], cacheTouchesAtlantic[MAXY][MAXX];
bool visited[MAXY][MAXX], inAns[MAXY][MAXX];
...
class Solution {
public:
    void dfs(int y, int x, vector<vector<int>>& matrix, int yUpper, int xUpper, vector<vector<int>> &ans) {
      if (visited[y][x]) return;
      
      if (y == 0 || x == 0) {
        cacheTouchesPacific[y][x] = true;
      }
      
      if (y == yUpper-1 || x == xUpper - 1) {
        cacheTouchesAtlantic[y][x] = true;
      }
      
      if (cacheTouchesPacific[y][x] && cacheTouchesAtlantic[y][x]) {
        if (!inAns[y][x]) {
          ans.push_back({y, x});
          inAns[y][x] = true;
        }
      ...
      for (int i = 0 ; i < 4; ++i) {
        ...
        int dx = dirx[i];
        int ny = y+dy, nx = x+dx;
        if (ny < 0 || nx < 0 || ny >= yUpper || nx >= xUpper) {
          continue;
        }
        if (matrix[ny][nx] > matrix[y][x]) {
          continue;
        }
        dfs(ny, nx, matrix, yUpper, xUpper, ans);
        
        cacheTouchesPacific[y][x] |= cacheTouchesPacific[ny][nx];
        cacheTouchesAtlantic[y][x] |= cacheTouchesAtlantic[ny][nx];
      }
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& matrix) {
      ...
      memset(cacheTouchesPacific, false, sizeof(cacheTouchesPacific));
      memset(cacheTouchesAtlantic, false, sizeof(cacheTouchesAtlantic));
      memset(inAns, false, sizeof(inAns));
      
      for (int y = 0; y < yUpper; ++y) {
        for (int x = 0; x < xUpper; ++x) {
          memset(visited, false, sizeof(visited));
          dfs(y, x, matrix, yUpper, xUpper, ans);
        }
      }
